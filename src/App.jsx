...
function MainApp() {
  ...
  const alerts = useAdjacencyAlerts(
    planogram.layout,
    skus,
    planogram.derivedParams,
  );
        ...
        <div className="flex items-center gap-4">
          ...
          {hasGenerated && planogram.config && (
            <span className="text-sm text-gray-500">
              {planogram.config.store_name} | {planogram.config.season} |{" "}
              {planogram.config.business_objective?.replace("_", " ")}
            </span>
          )}
        </div>
        ...
        <div className="flex-1 overflow-auto">
          {hasGenerated ? (
            <StoreCanvas
              ...
              onToggleBefore={() =>
                planogram.setShowBefore(!planogram.showBefore)
              }
            ...
            <div className="h-full flex items-center justify-center">
              <div className="text-center max-w-md">
                ...
                <p className="mt-2 text-sm text-gray-500">
                  Configure your store, objective, and season in the left panel,
                  then click
            ...
            <div className="border-t p-4">
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                <h4 className="text-xs font-semibold text-amber-800 flex items-center gap-1.5 mb-2">
                  ...
                  {planogram.skippedSKUs.length} SKU
                  {planogram.skippedSKUs.length !== 1 ? "s" : ""} not placed
                </h4>
                <p className="text-[11px] text-amber-700 mb-2">
                  Not enough zone space for these items:
                </p>
                <div className="space-y-1">
                  {planogram.skippedSKUs.map((s) => (
                    <div
                      key={s.sku_id}
                      className="text-[11px] text-amber-800 flex justify-between"
                    >
                      <span className="truncate font-medium">{s.sku_name}</span>
                      <span className="shrink-0 text-amber-600 ml-1">
                        {s.floor_space} sqft
                      </span>
                    </div>
                  ))}
