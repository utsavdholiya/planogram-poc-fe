...
export default function ExportModal({
  ...
  const billsAnalysed =
    derivedParams?.total_bills || derivedParams?.bills_analysed || "-";
  ...
  const layoutSummary = Object.entries(layout || {}).map(([zoneId, skuIds]) => {
    ...
    const names = (skuIds || []).map((id) => skuMap[id]?.sku_name || id);
    return {
  ...
  const handleExportPDF = useCallback(async () => {
    ...
    try {
      ...
      while (heightLeft > 0) {
        position = -(
          pageHeight * Math.ceil((imgHeight - heightLeft) / pageHeight)
        );
        pdf.addPage();
        pdf.addImage(imgData, "PNG", 0, position, imgWidth, imgHeight);
        heightLeft -= pageHeight;
              ...
              <>
                ...
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.86-3.572a4.5 4.5 0 00-1.242-7.244l-4.5-4.5a4.5 4.5 0 00-6.364 6.364L4.343 8.28"
                  />
                </svg>
                Copy Share Link
              </>
        ...
        <div className="px-6 pb-6">
          ...
          >
            ...
            <div className="border-t border-gray-200 pt-4 mt-8 flex items-center justify-between text-xs text-gray-400">
              <span>
                Generated by Planogram AI | Based on {billsAnalysed} bills
              </span>
