        ...
        <aside className="w-[200px] shrink-0 border-r border-gray-200 bg-gray-50 overflow-y-auto p-3">
          ...
          <div className="space-y-2">
            {categoryGroups.map((group) => {
              const border = CATEGORY_BORDER_COLORS[group.cat_id] || 'border-l-gray-300'
            ...
            <div>
              ...
              {config && (
                <p className="text-xs text-gray-500 mt-0.5">
                  {config.season} / {config.business_objective?.replace(/_/g, ' ')} / {config.campaign || 'no campaign'}
                </p>
              )}
            </div>
            <div className="flex items-center gap-2">
              {scoreDelta !== null && showBefore !== undefined && (
                <span
                  className={[
